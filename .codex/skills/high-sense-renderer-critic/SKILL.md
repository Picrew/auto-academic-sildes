...
3. Check these renderer-level signals:
   ...
   - `hard_layout`: reject `text-line-overlap`, `text-line-height-tight`, `text-self-overlap`, `title-wrap-too-deep`, `subtitle-wrap-too-deep`, `text-image-overlap`, `text-image-clearance-tight`, proof/artifact note-image collisions, `figure-caption-overlap`, `figure-caption-clearance-tight`, `container-overflow`, `missing-proof-image`, missing rendered image contracts, missing image crops, missing caption/source markers, `callout-outside-image`, unsupported image fit modes, severe letterboxing, excessive image upscaling, missing/generic image captions, low-readability caption styling, and image size errors before judging taste.
   - `image_contracts`: read the `Image Contracts` table in `layout-audit-report.md`; proof and artifact rows should have enough rendered pixels, meaningful visible area, healthy slot use, and crop/caption/source marked present.
   - `image_slot`: letterboxed proof/artifact images usually need a tighter crop or a different slot shape.
   - `useful_fill`: reject pages whose frames, borders, or whitespace occupy the slide while useful text/source evidence remains too sparse.
   - `artifact_role`: repo, UI, table, benchmark, paper, homepage, and project-page screenshots need larger slots than portraits, logos, or identity thumbnails.
...
- Treat `useful-fill-low` and `artifact-role-underfeatured` as delivery blockers in high-design review. The fix is usually a larger source slot, tighter crop, shorter side rail, or split slide.
